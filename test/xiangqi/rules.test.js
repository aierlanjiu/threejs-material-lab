...
import {
  ...
  posToIccs, iccsToPos, moveToString, stringToMove, boardToFen, fenToBoard, START_FEN,
} from '../../js/xiangqi/rules.js';
...
t('仕士只在九宫内斜行一步', () => {
  const s = createInitialState();
  const redAdvisor = pieceAt(s.pieces, 9, 3);
  assert.deepEqual(pseudoMoves(s.pieces, redAdvisor).map(x => `${x.row},${x.col}`), ['8,4']);
  assert.deepEqual(legalMoves(s.pieces, redAdvisor).map(x => `${x.row},${x.col}`), ['8,4']);

  const blackAdvisor = pieceAt(s.pieces, 0, 5);
  assert.deepEqual(pseudoMoves(s.pieces, blackAdvisor).map(x => `${x.row},${x.col}`), ['1,4']);

  s.pieces.push({ id: 99, type: 'soldier', color: RED, row: 8, col: 4 });
  assert.deepEqual(pseudoMoves(s.pieces, redAdvisor), []);
});
...
t('仕不能充当敌炮炮架让己方帅受将', () => {
  ...
  const advisor = pieces[3];
  assert.equal(isInCheck(pieces, RED), false);
  assert.deepEqual(pseudoMoves(pieces, advisor), [{ row: 8, col: 4 }]);
  assert.deepEqual(legalMoves(pieces, advisor), []);
});
...
// ---- FEN / ICCS ----
t('ICCS 坐标与着法转换', () => {
  assert.equal(posToIccs(7, 1), 'b2');
  assert.equal(posToIccs(0, 0), 'a9');
  assert.equal(posToIccs(9, 8), 'i0');
  assert.deepEqual(iccsToPos('b2'), { row: 7, col: 1 });
  assert.equal(moveToString({ row: 7, col: 1 }, { row: 7, col: 4 }), 'b2e2');
  assert.deepEqual(stringToMove('b2e2'), { from: { row: 7, col: 1 }, to: { row: 7, col: 4 } });
});
...
t('FEN 导出与导入互转', () => {
  ...
  const s2 = fenToBoard(fen);
  assert.equal(s2.pieces.length, 32);
  assert.equal(s2.turn, RED);
  assert.equal(boardToFen(s2), fen);
});
