...
export function posToIccs(row, col) {
  const c = String.fromCharCode(97 + col);
  const r = (9 - row).toString();
  return c + r;
}

export function iccsToPos(iccsStr) {
  if (!iccsStr || iccsStr.length < 2) return null;
  const col = iccsStr.charCodeAt(0) - 97;
  const row = 9 - parseInt(iccsStr[1], 10);
  return { row, col };
}

export function moveToString(from, to) {
  return posToIccs(from.row, from.col) + posToIccs(to.row, to.col);
}

export function stringToMove(iccsStr) {
  if (!iccsStr || iccsStr.length < 4) return null;
  const from = iccsToPos(iccsStr.slice(0, 2));
  const to = iccsToPos(iccsStr.slice(2, 4));
  return { from, to };
}
  ...
  for (let r = 0; r < ROWS; r++) {
    ...
    for (let c = 0; c < COLS; c++) {
      ...
      if (!p) {
        ...
        line += PIECE_TO_FEN[p.color][p.type];
      }
    }
    if (empty > 0) line += empty;
    rows.push(line);
  }
  const turnStr = state.turn === RED ? 'w' : 'b';
  return `${rows.join('/')} ${turnStr} - - 0 1`;
}

export function fenToBoard(fenStr) {
  if (!fenStr) return createInitialState();
  const parts = fenStr.trim().split(/\s+/);
  const posPart = parts[0];
  const turnPart = parts[1] || 'w';
  const rows = posPart.split('/');
  if (rows.length !== 10) return createInitialState();
  ...
  for (let r = 0; r < 10; r++) {
    ...
    for (const char of rows[r]) {
      if (char >= '1' && char <= '9') {
        ...
        const info = FEN_TO_PIECE[char];
        pieces.push({ id: id++, type: info.type, color: info.color, row: r, col: c });
        c++;
      }
    }
  }

  const turn = turnPart === 'b' ? BLACK : RED;
  return { pieces, turn, history: [], lastMove: null };
}
